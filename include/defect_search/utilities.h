...
int getMissingVertexColor(int vertex, const AdjacencyListGraph &G, const std::set<Edge> &M1, const std::set<Edge> &M2,
                          const std::set<Edge> &M3);

vector<Edge> findAlternatingPath(const AdjacencyListGraph &G, int start, int colorA, int colorB, const set<Edge> &M1,
                                 const set<Edge> &M2, const set<Edge> &M3);
